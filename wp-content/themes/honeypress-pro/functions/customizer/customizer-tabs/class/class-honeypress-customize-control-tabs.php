 ...
 * @access public
 */
class honeypress_Customize_Control_Tabs extends WP_Customize_Control {

	/**
	 * honeypress_Customize_Control_Tabs constructor.
	 *
	...
	public function render_content() {
		...
		$output .= '<div class="honeypress-tabs-control" id="input_' . esc_attr( $this->id ) . '">';
		foreach ( $this->tabs as $value => $args ) {
			if ( ! empty( $args['controls'] ) && ( $this->tab_has_controls( $args['controls'] ) ) ) {
				$controls_attribute = json_encode( $args['controls'] );

				$output .= '<div class="honeypress-customizer-tab">';
				...
				$output .= '</label>';
				$output .= '</div><!-- /.honeypress-customizer-tab -->';
			}
		}
		$output .= '</div><!-- /.honeypress-tabs-control -->';
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'honeypress-tabs-control-script', HONEYPRESS_PRO_TEMPLATE_DIR_URI . '/functions/customizer/customizer-tabs/js/script.js', array( 'jquery' ), 999, true );
		wp_enqueue_style( 'honeypress-tabs-control-style', HONEYPRESS_PRO_TEMPLATE_DIR_URI . '/functions/customizer/customizer-tabs/css/style.css', null, 999 );

	}

	/**
	 * Enqueue the partials handler script that works synchronously with the honeypress-tabs-control-script
	 */
	public function partials_helper_script_enqueue() {
		wp_enqueue_script( 'honeypress-tabs-addon-script', HONEYPRESS_PRO_TEMPLATE_DIR_URI . '/functions/customizer/customizer-tabs/js/customizer-addon-script.js', array( 'jquery' ), 999, true );
	}
