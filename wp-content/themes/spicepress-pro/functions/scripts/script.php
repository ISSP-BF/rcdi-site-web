...
if( !function_exists('spicepress_scripts_function'))
{
	function spicepress_scripts_function(){
		...
		wp_enqueue_style('default', ST_TEMPLATE_DIR_URI . '/css/'.$class);
		}
		...
		wp_enqueue_style('spicepress-light-css', ST_TEMPLATE_DIR_URI . '/css/lightbox.css');
		wp_enqueue_style('spicepress-font-awesome.min-css', ST_TEMPLATE_DIR_URI . '/css/font-awesome/css/font-awesome.min.css');
		...
		wp_enqueue_style('spicepress-owl-carousel-css', ST_TEMPLATE_DIR_URI . '/css/owl.carousel.css');
		if(get_theme_mod('spicepress_color_skin','light')=='dark')
		{
		wp_enqueue_style('spicepress-dark-css', ST_TEMPLATE_DIR_URI . '/css/dark.css');
		}
		
		
		wp_enqueue_script( 'jquery' );
		wp_enqueue_script('bootstrap-js', ST_TEMPLATE_DIR_URI . '/js/bootstrap.min.js');
		...
		{
		wp_enqueue_script('grid-masonary', ST_TEMPLATE_DIR_URI.'/js/grid-mansory.js', array('jquery'), '', true);
		wp_enqueue_script('imgLoad', get_template_directory_uri().'/js/img-loaded.js', array('jquery'), '', true);
		}	
		wp_enqueue_script('spicepress-page-scroll_js', ST_TEMPLATE_DIR_URI . '/js/page-scroll.js');
		wp_enqueue_script('owl-carousel-min-js', ST_TEMPLATE_DIR_URI . '/js/owl.carousel.min.js');
		wp_enqueue_script('spicepress-light-js', ST_TEMPLATE_DIR_URI . '/js/lightbox/lightbox-2.6.min.js');
		wp_enqueue_script('spicepress-mp-masonry-js', ST_TEMPLATE_DIR_URI . '/js/masonry/mp.mansory.js');
		wp_enqueue_script('spicepress-custom-scripts', ST_TEMPLATE_DIR_URI . '/js/custom_scripts.js',array('jquery'),'',true);
...
function spicepress_enqueue_scripts(){
	wp_enqueue_style('customize-css', ST_TEMPLATE_DIR_URI . '/css/customize.css');
}
add_action( 'admin_enqueue_scripts', 'spicepress_enqueue_scripts' );
...
if(is_page_template('template/template-blog-masonry-2-col.php'))
{
	
	$col =6;
}
elseif(is_page_template('template/template-blog-masonry-3-col.php'))
{
	
	$col =4;
}
elseif(is_page_template('template/template-blog-masonry-4-col.php'))
{
