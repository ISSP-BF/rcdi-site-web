...
class Breadcrumb_Trail {
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $heading_title ) {
			}
			$breadcrumb .= '<h1 class="text-white">'.$heading_title.'</h1>';
			// Open the unordered list.
			$breadcrumb .= sprintf(
				'<%s class="page-breadcrumb text-center" itemscope itemtype="http://schema.org/BreadcrumbList">',
			...
			foreach ( $this->items as $item ) {
				...
				// Build the list item.
				if(get_theme_mod('breadcrumb_setting_enable',true)==true):
