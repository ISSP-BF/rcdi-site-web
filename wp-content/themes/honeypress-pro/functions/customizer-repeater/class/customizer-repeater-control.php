...
class honeypress_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		if ( file_exists( HONEYPRESS_PRO_TEMPLATE_DIR . '/functions/customizer-repeater/inc/icons.php' ) ) {
	...
	public function enqueue() {
		wp_enqueue_style( 'font-awesome', HONEYPRESS_PRO_TEMPLATE_DIR_URI . '/css/font-awesome/css/font-awesome.min.css', array(), 999 );

		wp_enqueue_style( 'honeypress_customizer-repeater-admin-stylesheet', HONEYPRESS_PRO_TEMPLATE_DIR_URI . '/functions/customizer-repeater/css/admin-style.css', array(), 999 );

		wp_enqueue_style( 'wp-color-picker' );

		wp_enqueue_script( 'honeypress_customizer-repeater-script', HONEYPRESS_PRO_TEMPLATE_DIR_URI . '/functions/customizer-repeater/js/customizer_repeater.js', array('jquery', 'jquery-ui-draggable', 'wp-color-picker' ), 999, true  );

		wp_enqueue_script( 'honeypress_customizer-repeater-fontawesome-iconpicker', HONEYPRESS_PRO_TEMPLATE_DIR_URI . '/functions/customizer-repeater/js/fontawesome-iconpicker.js', array( 'jquery' ), 999, true );

		wp_enqueue_style( 'honeypress_customizer-repeater-fontawesome-iconpicker-script', HONEYPRESS_PRO_TEMPLATE_DIR_URI . '/functions/customizer-repeater/css/fontawesome-iconpicker.min.css', array(), 999 );
	}
			...
			foreach($array as $icon){ ?>
				<div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
					...
					<div class="customizer-repeater-box-content-hidden">
						<?php
						$choice = $image_url = $icon_value = $title = $subtitle = $text = $slide_format = $sidebar_check = $link = $designation = $home_slider_caption = $abtbutton_text = $abtlink = $abtopen_new_tab = $button = $open_new_tab = $shortcode = $repeater = $color = $video_url = $price = $features =$price_heighlight = '';
						if(!empty($icon->id)){
							$id = $icon->id;
						}
						...
						if($this->customizer_repeater_features_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Features','honeypress' ), $this->id, 'customizer_repeater_features_control' ),
								'class' => 'customizer-repeater-features-control',
								'type'  => apply_filters('honeypress_repeater_input_types_filter', 'textarea', $this->id, 'customizer_repeater_features_control' ),
							), $features);
						}
						...
						if($this->customizer_repeater_abtbutton_text_control){
							...
							'honeypress'), $this->id, 'customizer_repeater_abtbutton_text_control'),
						...
						if($this->customizer_repeater_abtlink_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Link2','honeypress' ), $this->id, 'customizer_repeater_abtlink_control' ),
								'class' => 'customizer-repeater-abtlink-control',
								'sanitize_callback' => 'esc_url_raw',
                                'type'  => apply_filters('honeypress_repeater_input_types_filter', '', $this->id, 'customizer_repeater_abtlink_control' ),
							), $abtlink);
						}
						...
						if($this->customizer_repeater_price_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Price','honeypress' ), $this->id, 'customizer_repeater_price_control' ),
								'class' => 'customizer-repeater-price-control',
                                'type'  => apply_filters('honeypress_repeater_input_types_filter', '', $this->id, 'customizer_repeater_price_control' ),
							), $price);
						}
...
//print_r($options);
	...
	<?php esc_html_e('Open link in new tab:','honeypress'); ?>
	<span class="switch">
	  <input type="checkbox" name="abtcustom_checkbox" value="yes" <?php if($value=='yes'){echo 'checked';}?> class="customizer-repeater-abtcheckbox" style="margin-bottom: 0 !important;">
	</span>
	</div>
	<?php
	}
	...
	private function sidebar_check($value='no'){
		?>
	<div class="customize-control-title">
	<?php esc_html_e('Enable Slider Widgets:','honeypress'); ?>
	<span class="switch">
	  ...
	  <label><?php esc_html_e('Note: Widgets will not apply with center content alignment','honeypress'); ?></label>
	</span>
	</div>
	<?php
	}
		...
		<div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
			...
			<span class="description customize-control-description">
                ...
                echo sprintf(
	                ...
	                sprintf( '<a href="http://fontawesome.io/icons/" rel="nofollow">%s</a>', esc_html__( 'http://fontawesome.io/icons/', 'honeypress' ) )
                ); ?>
            </span>
	...
	private function image_control($value = '', $show = ''){ ?>
		<div class="customizer-repeater-image-control" <?php if( $show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
			...
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_html_e( 'Upload Image','honeypress' ); ?>" />
		</div>
		<?php
	}
	...
	<span class="customize-control-title">
	<?php esc_html_e('Content Alignment','honeypress'); ?>
	</span>
	<select class="customizer-repeater-slide-caption-align">
	<option value="customizer_repeater_slide_caption_left" <?php if($value=='customizer_repeater_slide_caption_left') { echo 'selected';} ?>>
		<?php esc_html_e('Left','honeypress') ?>
		</option>
		
		<option value="customizer_repeater_slide_caption_center" <?php if($value=='customizer_repeater_slide_caption_center') { echo 'selected';} ?>>
		<?php esc_html_e('Center','honeypress') ?>
		</option>
		
		<option value="customizer_repeater_slide_caption_right" <?php if($value=='customizer_repeater_slide_caption_right') { echo 'selected';} ?>>
		<?php esc_html_e('Right','honeypress') ?>
		</option>
	...
	<span class="customize-control-title">
	<?php esc_html_e('Price highlight','honeypress'); ?>
	</span>
	<select class="customizer-repeater-price-heighlight">
		...
		<?php esc_html_e('Highlight','honeypress') ?>
		</option>
		...
		<?php esc_html_e('Non highlight','honeypress') ?>
		</option>
	</select>
			...
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<input type="text" class="customizer-repeater-social-repeater-link"
					       placeholder="<?php esc_html_e( 'Link', 'honeypress' ); ?>">
