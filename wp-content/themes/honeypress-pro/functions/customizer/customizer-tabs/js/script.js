...
var honeypress_customize_control_tabs = function ( $ ) {
	'use strict';
		...
		function () {
				...
				$( '.honeypress-tabs-control' ).each(
					function () {
						...
						var shownCtrls = $( this ).find( '.honeypress-customizer-tab > input:checked' ).data( 'controls' );
						showControls( customizerSection, shownCtrls );
					}
				);

				$( '.honeypress-customizer-tab > label' ).on(
					'click', function () {
						var customizerSection = $( this ).closest( '.accordion-section' );
...
honeypress_customize_control_tabs( jQuery );
