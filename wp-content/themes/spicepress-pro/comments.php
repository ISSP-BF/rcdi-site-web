...
<div <?php comment_class('media comment-box'); ?> id="comment-<?php comment_ID(); ?>">
	...
	<div class="media-body">
		<div class="comment-detail">
			<h6 class="comment-detail-title"><?php comment_author(); ?> <span class="comment-date"><?php esc_attr_e('Posted on ','spicepress'); ?><?php  echo comment_time(); ?><?php echo " - "; echo comment_date();?></span></h6>
			<?php comment_text(); ?>
...
<?php if ('open' == $post->comment_status) :
	if ( get_option('comment_registration') && !$user_ID ): ?>
		<p><?php echo sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment','spicepress' ), esc_url(site_url( 'wp-login.php' )) . '?redirect_to=' .  urlencode(get_permalink())); ?></p>
		<?php else:
	...
	$defaults = array(
		...
		'logged_in_as'=>'<p class="blog-post-info-detail">' . __( "Logged in as",'spicepress').' '.'<a href="'. admin_url( 'profile.php' ).'">'.$user_identity.'</a>'. ' <a href="'. wp_logout_url( get_permalink() ).'" title="'.__('Log out of this account','spicepress').'">'.__("Logout",'spicepress').'</a>' . '</p>',
